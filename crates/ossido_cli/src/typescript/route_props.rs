...
/// its `#[ossido::handler]` returns — but only for handlers that return a
/// concrete type (a `#[Props]` struct), not `Response`. Powers the
...
mod tests {
    ...
    fn extracts_concrete_handler_return_type_only() {
        ...
        assert_eq!(
            concrete_type_name(&syn::parse_str::<Type>("ossido::Response").unwrap()),
            None
        );
    }
}
