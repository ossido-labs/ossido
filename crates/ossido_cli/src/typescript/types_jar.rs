...
impl TypesJar {
    ...
    pub fn refresh_file(&mut self, path: PathBuf) {
        if let Ok(file_str) = read_to_string(&path) {
            if mentions_a_type_marker(&file_str) {
                ...
                // the user removed the attribute, so drop its types.
    ...
    fn log_duplicated_types(&self) {
        ...
        for (type_name, file_paths) in duplicates.iter() {
            ...
            ossido_println!("Duplicate \"{}\" type found in files:\n", type_name);
            ossido_println!("- {}", first_file_path);
            ossido_println!("- {}\n", second_file_path);
        }
    }
    ...
    pub fn generate_typescript_file(
        ...
        let typescript = self.generate_typescript(extra, trailing);
        let typescript_file_path = base_path.join(".ossido").join("types.ts");
