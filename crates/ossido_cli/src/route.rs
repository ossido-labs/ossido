...
/// (`[...catchall]`). Mirrors `ossido::SegmentValue`'s untagged JSON.
...
/// (imported or fully-qualified `#[ossido::api(METHOD)]`). An unrecognised
...
/// `#[ossido::static_paths]`) enumerator, parsing the AST so comments and
...
mod tests {
    ...
    fn parses_api_methods_in_both_imported_and_qualified_forms() {
        ...
        assert_eq!(
            parse_api_methods("#[ossido::api(POST)]\nasync fn h() {}"),
            vec![Method::POST]
        );
