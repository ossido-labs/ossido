...
impl MiddlewareData {
    ...
    // Reads a middleware.rs file and returns a Vector of Strings representing functions that were decorated with the ossido::middleware macro
...
mod tests {
    ...
    fn test_route_directory_info_new() {
        ...
        writeln!(file, "#[ossido::middleware]\nfn test_middleware() {{}}").unwrap();
    ...
    fn test_has_middleware_attr() {
        ...
        let attr: Attribute = syn::parse_quote!(#[ossido::middleware]);
        assert!(MiddlewareData::has_middleware_attr(&[attr]));

        // Imported form (`use ossido::middleware;` then `#[middleware]`).
