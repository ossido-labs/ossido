...
/// its `#[tuono_lib::handler]` returns — but only for handlers that return a
/// concrete type (a `#[derive(Props)]` struct), not `Response`. Powers the
/// generated `TuonoPage<'/path'>` helper.
...
/// the type its `layout.rs` handler returns. Powers `TuonoLayout<'/path'>`. The
...
/// Render the `RouteProps`/`TuonoPage` and `LayoutProps`/`TuonoLayout` helpers,
/// to be inserted inside the generated `declare module "tuono/types"` block.
...
mod tests {
    ...
    fn extracts_concrete_handler_return_type_only() {
        ...
        assert_eq!(
            concrete_type_name(&syn::parse_str::<Type>("tuono_lib::Response").unwrap()),
            None
        );
    }
}
