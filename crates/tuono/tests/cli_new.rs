...
async fn it_creates_a_new_project_and_replace_the_versions_in_the_manifest() {
    ...
    // The `.tuono` entry point is generated at scaffold time so the crate is
    // valid immediately (its `[[bin]]` points at `.tuono/main.rs`).
    let dot_tuono_main = temp_project
        .path()
        .join(project_folder)
        .join(".tuono")
        .join("main.rs");
    let dot_tuono_main_content =
        fs::read_to_string(&dot_tuono_main).expect(".tuono/main.rs was not generated");
    assert!(dot_tuono_main_content.contains("async fn main()"));

    let version = crate_version!();

    // The project name in both manifests is set to the target folder.
    let expected_cargo_toml_content =
        format!("[package] name = \"{project_folder}\" [dependencies] tuono_lib = \"{version}\"");

    assert_eq!(cargo_toml_content, expected_cargo_toml_content);

    let expected_package_json_content = format!(
        "{{\"name\": \"{project_folder}\", \"dependencies\": {{ \"tuono\": \"{version}\" }}}}"
    );
...
async fn it_patches_mdx_onto_the_base_template() {
    // The mock serves the `tuono-app` base. MDX is a patch, so it composes onto
    ...
    // MDX deps merged in, existing dep preserved.
    assert!(package_json.contains("\"@mdx-js/react\""));
    assert!(package_json.contains("\"@mdx-js/rollup\""));
    assert!(package_json.contains("\"tuono\""));
    // MDX is a patch, not a base swap — no Tailwind deps leak in.
    assert!(!package_json.contains("tailwindcss"));

    let config =
        fs::read_to_string(project_path.join("tuono.config.ts")).expect("tuono.config.ts missing");
...
async fn it_reports_an_actionable_error_when_the_version_tag_is_missing() {
    ...
    Mock::given(method("GET"))
        .and(path(format!(
            "repos/tuono-labs/tuono/git/ref/tags/v{version}"
        )))
        .respond_with(
    ...
    let _temp_project = TempTuonoProject::new();

    let assert = Command::cargo_bin("tuono")
        .unwrap()
        .arg("new")
        .arg("boom")
        .env("__INTERNAL_TUONO_TEST", server.uri())
        .assert()
        .failure();
