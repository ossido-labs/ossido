...
pub fn handler_core(_args: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let props_state_binding = if state_field_names.is_empty() {
        ...
        quote! {
            let crate::tuono_main_state::ApplicationState { #state_field_names, .. } = _state;
        }
    };
...
mod tests {
    ...
    fn generates_the_three_entry_points() {
        ...
        assert!(out.contains("asyncfntuono_internal_props"));
        assert!(out.contains("pubasyncfntuono_internal_route"));
        assert!(out.contains("pubasyncfntuono_internal_api"));
