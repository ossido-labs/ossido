...
pub fn handler_core(_args: TokenStream, item: TokenStream) -> TokenStream {
    ...
    quote! {
        ...
        pub async fn ossido_internal_route(
            ...
            use ossido::axum::response::IntoResponse as _;
           ...
           let req = ossido::Request::new(pathname.to_owned(), headers.to_owned(), params, None);
           #logger_bindings
           ...
           // type that derives `ossido::Props`.
           //
           // The non-`Send` `Response` (its props box a `dyn Serialize`) is turned
           ...
           match outcome {
               Ok(render_job) => ossido::finish_render(render_job).await.into_response(),
               // The error path runs the registered error handler (which may
               // override the response) before rendering the default error page.
               Err((req, server_error)) => ossido::render_error_to_string(req, server_error).await.into_response(),
           }
        }
        ...
        pub async fn ossido_internal_api(
           ...
           match outcome {
               ...
               // The error path runs the registered error handler (which may
               // override the response) before the default JSON 500.
               Err((req, server_error)) => ossido::respond_to_api_error(req, server_error).await.into_response(),
           }
        }
    }
}
