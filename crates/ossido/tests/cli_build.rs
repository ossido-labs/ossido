...
const POST_API_FILE: &str = "#[ossido_lib::api(POST)]\nasync fn post_handler() {}\n";
const GET_API_FILE: &str = "#[ossido_lib::api(GET)]\nasync fn get_handler() {}\n";
const HANDLER_FILE: &str = "#[ossido_lib::handler]\nasync fn handler(_req: ossido_lib::Request) -> ossido_lib::Response { todo!() }";

fn tracing_message(level: Level, module: &str, message: &str) -> String {
    format!("\x1b[31m{level}\x1b[0m \x1b[2mossido::{module}\x1b[0m\x1b[2m:\x1b[0m {message}\n")
}
...
fn it_successfully_import_mixed_case_routes() {
    ...
    for method in ["get", "post", "put", "delete", "patch"] {
        let expected = format!(r#"use ossido_lib::axum::routing::{method};"#);
...
fn it_successfully_composes_layout_handlers_into_pages() {
    ...
    // its route file path.
    assert_contains_ignoring_whitespace(&temp_main_rs_content, r#"ossido_lib::tokio::join!("#);
    assert_contains_ignoring_whitespace(
        &temp_main_rs_content,
...
const DYNAMIC_WITH_STATIC_PATHS: &str = "#[ossido_lib::handler]\nasync fn handler(_req: ossido_lib::Request) -> ossido_lib::Response { todo!() }\n#[ossido_lib::static_paths]\nasync fn static_paths(_paths: &mut ossido_lib::StaticPaths) {}\n";
