...
impl SourceBuilder {
    ...
    fn generate_axum_source(&self) -> String {
        ...
        for method in used_http_methods.into_iter() {
            ...
            import_http_handler.push_str(&format!("use ossido_lib::axum::routing::{method};\n"))
        }
        ...
        for (page_path, route) in &self.app.route_map {
            ...
            handlers.push_str(&format!(
                ...
                async fn __ossido_ssr_{page_module}(
                    ossido_lib::axum::extract::Path(params): ossido_lib::axum::extract::Path<std::collections::HashMap<String, String>>,
                    ossido_lib::axum::extract::State(state): ossido_lib::axum::extract::State<crate::ossido_main_state::ApplicationState>,
                    request: ossido_lib::axum::extract::Request,
                ) -> impl ossido_lib::axum::response::IntoResponse {{
                    let req = ossido_lib::Request::new(request.uri().to_owned(), request.headers().to_owned(), params, None);
                    let ({bindings}) = ossido_lib::tokio::join!(
                        {join_futures}
                    );
                    let layouts = vec![{layout_entries}];
                    ossido_lib::render_chain(req, page, layouts).await
                }}
                ...
                async fn __ossido_data_{page_module}(
                    ...
                    let layouts = vec![{layout_entries}];
                    ossido_lib::chain_json(page, layouts)
                }}
                "#
            ));
        }
...
mod tests {
    ...
    fn should_not_load_the_axum_get_function() {
        ...
        assert!(!dev_bundle.contains("use ossido_lib::axum::routing::get;"));
    }
