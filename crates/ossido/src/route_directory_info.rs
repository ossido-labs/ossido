...
impl MiddlewareData {
    ...
    pub fn has_middleware_attr(attrs: &[Attribute]) -> bool {
        attrs
            .iter()
            .any(|attr| crate::macro_attr::is_ossido_attr(attr.path(), "middleware"))
    }

    // Reads a middleware.rs file and returns a Vector of Strings representing functions that were decorated with the ossido_lib::middleware macro
...
mod tests {
    ...
    fn test_route_directory_info_new() {
        ...
        writeln!(file, "#[ossido_lib::middleware]\nfn test_middleware() {{}}").unwrap();
    ...
    fn test_has_middleware_attr() {
        ...
        let attr: Attribute = syn::parse_quote!(#[ossido_lib::middleware]);
        assert!(MiddlewareData::has_middleware_attr(&[attr]));

        // Imported form (`use ossido_lib::middleware;` then `#[middleware]`).
