...
async fn handle_socket(mut ossido_socket: WebSocket) {
    // Send back a message to confirm connection
    if ossido_socket
        .send(ws::Message::Ping("ossido connected".into()))
        .await
        .is_err()
    {
    ...
    let (mut ossido_sender, mut ossido_receiver) = ossido_socket.split();

    // Handle browser messages.
    // Every message gets forwarded to the vite WebSocket
    tokio::spawn(async move {
        while let Some(msg) = ossido_receiver.next().await {
            if let Ok(msg) = msg {
    ...
    tokio::spawn(async move {
        while let Some(Ok(msg)) = vite_receiver.next().await {
            ...
            if ossido_sender.send(msg_to_browser).await.is_err() {
                break;
            }
        }
    });
