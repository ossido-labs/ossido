...
//! form (`#[handler]`) and the fully-qualified form (`#[ossido_lib::handler]`).
...
/// form (`#[name]`) or the fully-qualified `#[ossido_lib::name]`.
pub fn is_ossido_attr(path: &Path, name: &str) -> bool {
    ...
    match segments.as_slice() {
        [only] => *only == name,
        [krate, attr] => *krate == "ossido_lib" && *attr == name,
        _ => false,
    }
}
...
mod tests {
    ...
    fn matches_imported_and_fully_qualified_forms() {
        ...
        let qualified: Path = parse_quote!(ossido_lib::handler);
    ...
    fn rejects_other_names_crates_and_shapes() {
        ...
        // Too many segments.
        let deep: Path = parse_quote!(a::ossido_lib::handler);
