...
pub async fn render_error_to_string(req: Request, error: ServerError) -> AxumResponse {
    let error_id = crate::error_handler::next_error_id();
    if let Some(custom) = crate::error_handler::run_error_handler(&req, &error, &error_id).await {
        // The registered handler took over the response; still surface the error
        // in the server console (the render paths below would otherwise log it).
        error.log();
        return custom;
    }
...
/// Respond to a panicked API/data handler: run the registered error handler
/// first (it may override the response), otherwise fall back to [`error_json`].
pub async fn respond_to_api_error(req: Request, error: ServerError) -> AxumResponse {
    let error_id = crate::error_handler::next_error_id();
    if let Some(custom) = crate::error_handler::run_error_handler(&req, &error, &error_id).await {
        error.log();
        return custom;
...
/// the generated `ossido_internal_props` so the value returned across `await`
/// boundaries is `Send`.
