...
//! — Tuono's answer to Next's `getStaticPaths` / `generateStaticParams`.
//!
//! A `#[tuono_lib::static_paths]` function in a dynamic route's `page.rs`
...
//! `tuono build --static` queries to learn which concrete URLs to render. The
...
/// `#[tuono_lib::static_paths]` function receives this borrowed and `register`s
/// one entry per page.
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
