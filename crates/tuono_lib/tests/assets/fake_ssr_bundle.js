// Minimal stand-in for a built tuono `prod-server.js`, used by the tuono_lib
...
// Playwright e2e suite, which runs against a real `tuono build`.
//
...
//                             the `__ssr_write` global tuono installs. Driven by
...
// `window.__TUONO_SERVER_PAYLOAD__`, mirroring the real bundle closely enough
...
;(function (exports) {
  ...
  function renderDocument(payload) {
    ...
    return (
      ...
      '<main><div id="app">tuono ssr fixture</div></main>' +
      '<script>window.__TUONO_SERVER_PAYLOAD__ = ' +
      (raw || 'null') +
      '</script>' +
      '</body></html>'
    )
