...
//! (`Ssr::render`) — the two ssr_rs render paths tuono uses. Benchmarking at the
//! `Ssr` boundary isolates the difference that matters; the tuono render-pool
//! layer is identical plumbing on top of both.
//!
//! Run with:
//!   cargo bench -p tuono_lib --bench streaming_vs_buffered
//!
...
// ---------------------------------------------------------------------------
// Bench bundle: exposes the same contract as a real tuono SSR bundle, producing
...
/// ssr_rs's opt-in `Ssr::streaming` helper (the same path tuono takes).
fn stream(ssr: &mut Ssr<'static, 'static>, payload: &str, sink: impl FnMut(String) + 'static) {
    ssr.streaming("__ssr_write")
        .unwrap()
        .render("renderStream", Some(payload), sink)
        .unwrap();
}
...
fn main() {
    ...
    for &size in &SIZES {
        ...
        //    (The sink is owned + `'static`, so it captures nothing here.)
