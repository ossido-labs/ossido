...
/// Derive `From<Self> for ossido_lib::Response`, so a struct can be returned
/// directly from a `#[ossido_lib::handler]` instead of wrapping it in
/// `Response::Props(Props::new(..))`.
///
/// ```ignore
/// #[derive(serde::Serialize, ossido_lib::Props)]
...
pub fn derive_props_core(item: TokenStream) -> TokenStream {
    ...
    quote! {
        impl #impl_generics ::core::convert::From<#name #ty_generics>
            for ossido_lib::Response #where_clause
        {
            fn from(value: #name #ty_generics) -> Self {
                ossido_lib::Response::Props(ossido_lib::Props::new(value))
            }
        }
    }
}
...
mod tests {
    ...
    fn derives_a_from_impl_into_a_props_response() {
        ...
        assert!(out.contains("::core::convert::From<HomeProps>forossido_lib::Response"));
        assert!(out.contains("ossido_lib::Response::Props(ossido_lib::Props::new(value))"));
    }
