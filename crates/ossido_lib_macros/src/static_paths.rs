...
/// `#[ossido_lib::static_paths]` — mark the function in a dynamic route's
...
///
/// The macro wraps it in an axum handler (`ossido_internal_static_paths`) that
/// the codegen mounts on an internal endpoint; `ossido build --static` queries it
...
pub fn static_paths_core(_args: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let logger_bindings = if needs_request {
        quote! {
            let req = ossido_lib::Request::new(
    ...
    quote! {
        ...
        pub async fn ossido_internal_static_paths(
            #axum_arguments
            ...
            let mut paths = ossido_lib::StaticPaths::new();
            #fn_name(&mut paths, #argument_names).await;
            ossido_lib::axum::Json(paths)
        }
    }
}
...
mod tests {
    ...
    fn generates_the_wrapper_and_preserves_the_original_fn() {
        ...
        assert!(out.contains("pubasyncfnossido_internal_static_paths"));
        // The accumulator is created and forwarded by reference.
        assert!(out.contains("letmutpaths=ossido_lib::StaticPaths::new()"));
        assert!(out.contains("static_paths(&mutpaths,)"));
        assert!(out.contains("ossido_lib::axum::Json(paths)"));
    ...
    fn a_parameterless_enumerator_has_no_state_or_request_extractor() {
        ...
        assert!(!out.contains("request:ossido_lib::axum::extract::Request"));
    }
    ...
    fn a_logger_is_bound_against_a_synthesised_request() {
        ...
        assert!(out.contains("request:ossido_lib::axum::extract::Request"));
        assert!(out.contains("letreq=ossido_lib::Request::new"));
