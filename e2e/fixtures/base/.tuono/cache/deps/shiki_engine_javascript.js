//#region ../../../node_modules/.bun/@shikijs+engine-javascript@4.4.1/node_modules/@shikijs/engine-javascript/dist/scanner-DX8LRFGE.mjs
var MAX = 4294967295;
  ...
  findNextMatchSync(string, startPosition, _options) {
    ...
    if (pending.length) {
      ...
      for (const [i, match, offset] of pending)
        if (match.index === minIndex) return toResult(i, match, offset);
    }
    return null;
  }
};
...
function M$1(e, n = {}) {
  ...
  for (C$1.lastIndex = 0; (i = C$1.exec(e)); ) {
    ...
    (u.tokens ? r.push(...u.tokens) : u.token && r.push(u.token),
      u.lastIndex !== void 0 && (C$1.lastIndex = u.lastIndex));
  }
  const l = [];
  let c = 0;
  (r
    .filter((u) => u.type === 'GroupOpen')
    .forEach((u) => {
...
function F$1(e, n, t, o) {
  ...
  if (s === '\\') {
    ...
    if (a === 'N' || a === 'R')
      return { token: k$1('newline', t, { negate: a === 'N' }) };
  ...
  if (s === '(') {
    ...
    throw t === '(?('
      ? /* @__PURE__ */ new Error(`Unsupported conditional "${t}"`)
      : /* @__PURE__ */ new Error(`Invalid or unsupported group option "${t}"`);
  }
  if (t === ')') {
...
function x$1(e, { inCharClass: n }) {
  ...
  if (e.startsWith(o$1`\o{`))
    throw new Error(
      `Incomplete, invalid, or unsupported octal code point "${e}"`,
    );
  if (/^\\[pP]\{/.test(e)) {
    if (e.length === 3)
  ...
  if (/^\\x[89A-Fa-f]\p{AHex}/u.test(e))
    try {
      ...
      throw new Error(
        `Multibyte code "${e}" incomplete or invalid in Oniguruma`,
      );
    }
...
function L$1(e, n) {
  let { on: t, off: o } = /^\(\?(?<on>[imx]*)(?:-(?<off>[-imx]*))?/.exec(
    e,
  ).groups;
  o ??= '';
  const s = (n.getCurrentModX() || t.includes('x')) && !o.includes('x'),
    a = v(t),
    r = v(o),
    i = {};
  if ((a && (i.enable = a), r && (i.disable = r), e.endsWith(')')))
    return (n.replaceCurrentModX(s), I$1('flags', e, { flags: i }));
  if (e.endsWith(':'))
    return (
      n.pushModX(s),
      n.numOpenGroups++,
      f('group', e, { ...((a || r) && { flags: i }) })
    );
...
function j(e) {
  ...
  const a = s
      ...
      : [],
    [r, i, l] = a,
    c = B$1.has(t) ? t.toLowerCase() : 'custom';
  switch (c) {
    case 'fail':
    case 'mismatch':
    case 'skip':
    ...
    case 'error':
      ...
      if (typeof r == 'string')
        throw new Error(`Named callout argument must be a number "${r}"`);
      break;
    case 'max':
      if (!a.length || a.length > 2)
        throw new Error(`Named callout must have one or two arguments "${a}"`);
      if (typeof r == 'string' && !/^[A-Za-z_]\w*$/.test(r))
        throw new Error(
          `Named callout argument one must be a tag or number "${r}"`,
        );
      if (a.length === 2 && (typeof i == 'number' || !/^[<>X]$/.test(i)))
        throw new Error(
          `Named callout optional argument two must be '<', '>', or 'X' "${i}"`,
        );
      break;
    case 'count':
    case 'total_count':
      if (a.length > 1)
        throw new Error(`Named callout allows only one argument "${a}"`);
      if (a.length === 1 && (typeof r == 'number' || !/^[<>X]$/.test(r)))
        throw new Error(
          `Named callout optional argument must be '<', '>', or 'X' "${r}"`,
        );
      break;
    case 'cmp':
      if (a.length !== 3)
      ...
      if (typeof l == 'string' && !/^[A-Za-z_]\w*$/.test(l))
        throw new Error(
          `Named callout argument three must be a tag or number "${l}"`,
        );
      break;
    case 'custom':
...
function O$1(e) {
  let n = null,
    ...
    o;
  if (e[0] === '{') {
    const { minStr: s, maxStr: a } =
        /^\{(?<minStr>\d*)(?:,(?<maxStr>\d*))?/.exec(e).groups,
      r = 1e5;
    if (+s > r || (a && +a > r))
      throw new Error('Quantifier value unsupported in Oniguruma');
    if (
      ((t = +s),
      (o = a === void 0 ? +s : a === '' ? Infinity : +a),
      t > o && ((n = 'possessive'), ([t, o] = [o, t])),
      e.endsWith('?'))
    ) {
      if (n === 'possessive')
        throw new Error(
    ...
    ((t = e[0] === '+' ? 1 : 0),
      (o = e[0] === '?' ? 1 : Infinity),
      (n = e[1] === '+' ? 'possessive' : e[1] === '?' ? 'lazy' : 'greedy'));
  return _$1(n, t, o, e);
}
function q(e) {
...
function v(e) {
  const n = {};
  return (
    e.includes('i') && (n.ignoreCase = !0),
    e.includes('m') && (n.dotAll = !0),
    e.includes('x') && (n.extended = !0),
...
function ee$1(e, n) {
  const { raw: t, inCharClass: o } = e,
    s = t.slice(1);
  if (!o && ((s !== '0' && s.length === 1) || (s[0] !== '0' && +s <= n)))
    return [A$1(t)];
  const a = [],
    r = s.match(/^[0-7]+|\d/g);
  for (let i = 0; i < r.length; i++) {
    ...
    if (i === 0 && l !== '8' && l !== '9') {
      if (((c = parseInt(l, 8)), c > 127))
        throw new Error(o$1`Octal encoded byte above 177 unsupported "${t}"`);
    } else c = r$2(l);
    a.push(d(c, (i === 0 ? '\\' : '') + l));
  }
  return a;
...
function J(e, r = {}) {
    ...
    i = (p, N) => {
      ...
      switch (((t.parent = p), t.nextIndex++, u.type)) {
        ...
        case 'Backreference':
          return X(u, t);
        case 'Character':
          return m(u.value, { useLastValid: !!N.isCheckingRangeEnd });
  ...
  if (l && c.size && !n.rules.captureGroup)
    throw new Error(
  ...
  for (const { ref: p } of f)
    if (typeof p == 'number') {
      ...
      if (c.get(p).length > 1)
        throw new Error(o$1`Subroutine uses a duplicate group name "\g<${p}>"`);
      c.get(p)[0].isSubroutined = !0;
    } else
      throw new Error(
        o$1`Subroutine uses a group name that's not defined "\g<${p}>"`,
      );
  return d;
}
function W({ kind: e }) {
  return F(
    ...
    { negate: e === o$1`\B` || e === o$1`\Y` },
  );
}
function X({ raw: e }, r) {
  const n = /^\\k[<']/.test(e),
    o = n ? e.slice(3, -1) : e.slice(1),
    i = (t, d = !1) => {
      const s = r.capturingGroups.length;
      let a = !1;
      if (t > s)
        if (r.skipBackrefValidation) a = !0;
        else
          throw new Error(
            `Not enough capturing groups defined to the left "${e}"`,
          );
...
function ee(e, r, n) {
  ...
  if (
    ...
    if (d.type === 'Character' && a.type === 'Character')
      return (t.body.pop(), L(d, a));
...
function re({ negate: e }, r, n) {
  ...
  return (
    t.length === 1
      ? (a.body = t[0].body)
      : ((a.kind = 'intersection'),
        (a.body = t.map((l) => (l.body.length === 1 ? l.body[0] : l)))),
    r.nextIndex++,
    a
  );
}
...
function te(e, r, n) {
  const {
      ...
      capturingGroups: i,
      namedGroupsByName: t,
      skipLookbehindValidation: d,
      walk: s,
    } = r,
    a = ie(e),
    l = a.type === 'AbsenceFunction',
    c = $(a),
    f = c && a.negate;
  if (
    (a.type === 'CapturingGroup' &&
      (i.push(a), a.name && l$1(t, a.name, []).push(a)),
...
function m(e, r) {
  ...
  if (e > 1114111) {
    ...
    if (n.useLastValid) e = 1114111;
      ...
      throw e > 1310719
        ? /* @__PURE__ */ new Error(
            `Invalid code point out of range "\\x{${o}}"`,
          )
        : /* @__PURE__ */ new Error(
            `Invalid code point out of range in JS "\\x{${o}}"`,
          );
  }
  return {
    type: 'Character',
...
function Q(e, r) {
  ...
  if (!o) {
    if (n.normalizeUnknownPropertyNames) o = de(e);
      ...
      throw new Error(o$1`Invalid Unicode property "\p{${e}}"`);
  }
  return {
    type: 'CharacterSet',
    kind: 'property',
...
function z(e, r) {
  ...
  return u(
    e,
    `Unclosed character class${n?.type === 'Character' && n.value === 93 && n.raw === ']' ? ' (started with "]")' : ''}`,
  );
}
function D(e) {
  ...
  function n(e, s = null, t = null, r = null) {
    ...
    const d = {
        ...
        replaceWith(o, m = {}) {
          const y = !!m.traverse;
          (r
            ? (r[Math.max(0, l(t) + i)] = o)
            : (u(s, "Can't replace root node")[t] = o),
            y && n(o, s, t, r),
            (c = !0));
        },
        replaceWithMultiple(o, m = {}) {
          const y = !!m.traverse;
          if (
            (x(r).splice(Math.max(0, l(t) + i), 1, ...o),
            (i += o.length - 1),
            y)
          ) {
            let g = 0;
      ...
      u$1 = v['*'],
      h = v[f],
      R = typeof u$1 == 'function' ? u$1 : u$1?.enter,
      P = typeof h == 'function' ? h : h?.enter;
    if ((R?.(d, N), P?.(d, N), !c))
      switch (f) {
...
function replaceUnescaped(expression, needle, replacement, context) {
  ...
  for (const match of expression.matchAll(re)) {
    ...
    if (
      !$skip &&
      (!context || (context === Context.DEFAULT) === !numCharClassesOpen)
    ) {
...
*/
var atomicPluginToken = new RegExp(
  String.raw`(?<noncapturingStart>${noncapturingDelim})|(?<capturingStart>\((?:\?<[^>]+>)?)|\\?.`,
  'gsu',
);
/**
  ...
  do {
    ...
    while ((match = atomicPluginToken.exec(expression))) {
      const {
        0: m,
        index,
        groups: { capturingStart, noncapturingStart },
      } = match;
      if (m === '[') numCharClassesOpen++;
      else if (!numCharClassesOpen) {
        if (m === aGDelim && !inAG) {
...
var possessivePluginToken = new RegExp(
  String.raw`
\\(?: \d+
  | c[A-Za-z]
  | [gk]<[^>]+>
      ...
      if (qMod === '+' && lastToken && !lastToken.startsWith('(')) {
        ...
        if (/^\{\d+\}$/.test(qBase))
          ...
          if (lastToken === ')' || lastToken === ']') {
            const nodeIndex =
              lastToken === ')' ? lastGroupIndex : lastCharClassIndex;
            if (nodeIndex === null)
        ...
        lastGroupIndex = openGroupIndices.length
          ? openGroupIndices.pop()
          : null;
    }
    lastToken = m;
  }
...
var token = new RegExp(
  r$1`${namedCaptureDelim}|${recursiveToken}|\(\?|\\?.`,
  'gsu',
);
...
function recursion(pattern, data) {
  ...
  if (
    mode === 'plugin' &&
    hasUnescaped(pattern, r$1`\(\?\(DEFINE\)`, Context.DEFAULT)
  )
    throw new Error('DEFINE groups cannot be used with recursion');
  const addedHiddenCaptures = [];
  const hasNumberedBackref = hasUnescaped(
    pattern,
    r$1`\\[1-9]`,
    Context.DEFAULT,
  );
  ...
  while ((match = token.exec(pattern))) {
    const {
      0: m,
      groups: { captureName, rDepth, gRNameOrNum, gRDepth },
    } = match;
    if (m === '[') numCharClassesOpen++;
    else if (!numCharClassesOpen) {
      if (rDepth) {
        ...
        if (!isWithinReffedGroup)
          throw new Error(
            r$1`Recursive \g cannot be used outside the referenced group "${mode === 'external' ? gRNameOrNum : r$1`\g<${gRNameOrNum}&R=${gRDepth}>`}"`,
          );
        ...
        const groupContentsRight = groupContents.slice(
          groupContentsLeft.length + m.length,
        );
        ...
        token.lastIndex +=
          expansion.length -
          m.length -
          groupContentsLeft.length -
          groupContentsRight.length;
...
function repeatWithDepth(
  ...
  const startNum = 2;
  const getDepthNum = (i) =>
    direction === 'forward' ? i + startNum : reps - i + startNum - 1;
  let result = '';
    ...
    result += replaceUnescaped(
      ...
      ({ 0: m, groups: { captureName, unnamed, backref } }) => {
        ...
        if (unnamed || captureName) {
          const addedCaptureNum =
            numCapturesPassed + addedHiddenCaptures.length + 1;
          addedHiddenCaptures.push(addedCaptureNum);
...
function mapCaptureTransfers(
  ...
  if (captureTransfers.size && numCapturesAddedInExpansion) {
    ...
    const recursionDelimCaptureNum =
      numCapturesPassed -
      numCapturesInLeft +
      numAddedHiddenCapturesPreExpansion;
    const newCaptureTransfers = /* @__PURE__ */ new Map();
    captureTransfers.forEach((from, to) => {
      const numCapturesInRight =
        (numCapturesAddedInExpansion - numCapturesInLeft * reps) / reps;
      const numCapturesAddedInLeft = numCapturesInLeft * reps;
      const newTo =
        to > recursionDelimCaptureNum + numCapturesInLeft
          ? to + numCapturesAddedInExpansion
          : to;
      const newFrom = [];
      for (const f of from)
        if (f <= recursionDelimCaptureNum) newFrom.push(f);
        else if (
          f >
          recursionDelimCaptureNum + numCapturesInLeft + numCapturesInRight
        )
...
envFlags.bugFlagVLiteralHyphenIsRange = envFlags.unicodeSets
  ...
  : false;
envFlags.bugNestedClassIgnoresNegation =
  envFlags.unicodeSets && new globalRegExp('[[^a]]', 'v').test('a');
function getNewCurrentFlags(current, { enable, disable }) {
  return {
    dotAll: !disable?.dotAll && !!(enable?.dotAll || current.dotAll),
    ignoreCase:
      !disable?.ignoreCase && !!(enable?.ignoreCase || current.ignoreCase),
  };
}
...
function getOptions(options = {}) {
  ...
  if (opts.target === 'auto')
    opts.target = envFlags.flagGroups
      ? 'ES2025'
      : envFlags.unicodeSets
        ? 'ES2024'
        : 'ES2018';
...
var FirstPassVisitor = {
  ...
  Alternative: {
    enter({ node, parent, key }, { flagDirectivesByAlt }) {
      ...
      for (let i = key + 1; i < parent.body.length; i++) {
        ...
        getOrInsert(flagDirectivesByAlt, forwardSiblingAlt, []).push(
          ...flagDirectives,
        );
      }
    },
    exit({ node }, { flagDirectivesByAlt }) {
      if (flagDirectivesByAlt.get(node)?.length) {
        const flags = getCombinedFlagModsFromFlagNodes(
          flagDirectivesByAlt.get(node),
        );
        if (flags) {
  ...
  Assertion(
    { node, parent, key, container, root, remove, replaceWith },
    state,
  ) {
    const { kind, negate } = node;
    const { asciiWordBoundaries, avoidSubclass, supportedGNodes, wordIsAscii } =
      state;
    if (kind === 'text_segment_boundary')
      throw new Error(
        `Unsupported text segment boundary "\\${negate ? 'Y' : 'y'}"`,
      );
    else if (kind === 'line_end')
      replaceWith(
        setParentDeep(
          K({ body: [b({ body: [F('string_end')] }), b({ body: [m(10)] })] }),
          parent,
        ),
      );
  ...
  CharacterClassRange({ node, parent, replaceWith }) {
    if (parent.kind === 'intersection')
      replaceWith(setParentDeep(C({ body: [node] }), parent), {
        traverse: true,
      });
  },
  CharacterSet(
    { node, parent, replaceWith },
    { accuracy, minTargetEs2024, digitIsAscii, spaceIsAscii, wordIsAscii },
  ) {
    ...
    if (spaceIsAscii && (kind === 'space' || value === 'space')) {
      replaceWith(
        setParentDeep(setNegate(parseFragment(asciiSpaceChar), negate), parent),
      );
      return;
    }
    ...
    if (kind === 'any') replaceWith(setParent(Q('Any'), parent));
      ...
      if (!minTargetEs2024 && (value === 'graph' || value === 'print')) {
        if (accuracy === 'strict')
          throw new Error(
            `POSIX class "${value}" requires min target ES2024 or non-strict accuracy`,
          );
        let ascii = {
          graph: '!-~',
          print: ' -~',
        ...
        replaceWith(
          setParentDeep(
            setNegate(parseFragment(PosixClassMap.get(value)), negate),
            parent,
          ),
        );
      ...
      replaceWith(
        setParentDeep(
          setNegate(parseFragment(defaultWordChar), negate),
          parent,
        ),
      );
  ...
  Regex: {
    ...
    exit(_, { accuracy, passedLookbehind, strategy }) {
      if (accuracy === 'strict' && passedLookbehind && strategy)
        throw new Error(
          r`Uses "\G" in a way that requires non-strict accuracy`,
        );
    },
  },
...
var SecondPassVisitor = {
  Backreference(
    { node },
    { multiplexCapturesToLeftByRef, reffedNodesByReferencer },
  ) {
  ...
  CapturingGroup: {
    enter(
      ...
      const multiplexNodes = multiplexCapturesToLeftByRef.get(
        node.name ?? node.number,
      );
      for (let i = 0; i < multiplexNodes.length; i++) {
        const multiplex = multiplexNodes[i];
        if (
          origin === multiplex.node ||
          (origin && origin === multiplex.origin) ||
      ...
      if (node.name) {
        const groupsWithSameName = getOrInsert(
          groupsByName,
          node.name,
          /* @__PURE__ */ new Map(),
        );
  ...
  Subroutine({ node, parent, replaceWith }, state) {
    ...
    if (isRecursive) {
      ...
      while ((reffed = reffed.parent))
        if (
          reffed.type === 'CapturingGroup' &&
          (reffed.name === ref || reffed.number === ref)
        )
          break;
    ...
    if (!isGlobalRecursion) {
      const reffedGroupFlagMods = getCombinedFlagModsFromFlagNodes(
        getAllParents(reffedGroupNode, (p) => p.type === 'Group' && !!p.flags),
      );
...
var ThirdPassVisitor = {
  Backreference({ node, parent, replaceWith }, state) {
    if (node.orphan) {
      state.highestOrphanBackref = Math.max(
        state.highestOrphanBackref,
        node.ref,
      );
      return;
    }
    const participants = state.reffedNodesByReferencer
      .get(node)
      .filter((reffed) => canParticipateWithNode(reffed, node));
      ...
      replaceWith(
        setParentDeep(
          A({
            ...
            body: participants
              .reverse()
              .map((reffed) => b({ body: [k(reffed.number)] })),
          }),
          parent,
        ),
      );
  ...
  Regex: {
    exit({ node }, state) {
      const numCapsNeeded = Math.max(
        state.highestOrphanBackref - state.numCapturesToLeft,
        0,
      );
...
function cloneCapturingGroup(obj, originMap, up, up2) {
  ...
  for (const [key, value] of Object.entries(obj))
    if (key === 'parent') store.parent = Array.isArray(up) ? up2 : up;
...
function getAllParents(node, filterFn) {
  ...
  while ((node = node.parent))
    if (!filterFn || filterFn(node)) results.push(node);
  return results;
}
...
function getLeadingG(els) {
  const firstToConsider = els.find(
    (el) =>
  ...
  if (
    firstToConsider.type === 'CapturingGroup' ||
    firstToConsider.type === 'Group'
  ) {
...
function isAncestorOf(node, descendant) {
  ...
  for (const kid of kids)
    if (kid === descendant || isAncestorOf(kid, descendant)) return true;
  return false;
}
...
function isAlwaysNonZeroLength(node) {
  const types = ['Character', 'CharacterClass', 'CharacterSet'];
  return (
    types.includes(node.type) ||
    (node.type === 'Quantifier' && node.min && types.includes(node.body.type))
  );
}
...
function generate(ast, options) {
  ...
  const appliedGlobalFlags = {
    dotAll: ast.flags.dotAll,
    ignoreCase: !!(
      (ast.flags.ignoreCase || hasCaseInsensitiveNode) &&
      !hasCaseSensitiveNode
    ),
  };
  let lastNode = ast;
  const state = {
  ...
  '*': {
    enter({ node }, state) {
      if (isAnyGroup(node)) {
        ...
        state.pushModI(
          node.flags
            ? getNewCurrentFlags({ ignoreCase: currentModI }, node.flags)
                .ignoreCase
            : currentModI,
        );
      }
...
var generator = {
  ...
  CharacterClass(node, state, gen) {
    ...
    if (
      ...
      state.useFlagV &&
      body.some(isLiteralHyphen)
    )
      body = [m(45), ...body.filter((kid) => !isLiteralHyphen(kid))];
    const genClass = () =>
      `[${negate ? '^' : ''}${body.map(gen).join(kind === 'intersection' ? '&&' : '')}]`;
    if (!state.inCharClass) {
      if (
        ...
        const negatedChildClasses = body.filter(
          (kid) =>
            kid.type === 'CharacterClass' && kid.kind === 'union' && kid.negate,
        );
  ...
  CharacterClassRange(node, state) {
    ...
    if (state.useAppliedIgnoreCase && state.currentFlags.ignoreCase)
      getCodePointRangesFromChars(getCasesOutsideCharClassRange(node)).forEach(
        (value) => {
          extraChars.add(
            Array.isArray(value)
  ...
  CharacterSet({ kind, negate, value, key }, state) {
    ...
    if (kind === 'property') {
      if (
        ...
        throw new Error(
          `Unicode property "${value}" can't be case-insensitive when other chars have specific case`,
        );
  ...
  Group({ atomic: atomic2, body, flags, parent }, state, gen) {
    ...
    const result =
      ...
      (!state.useFlagMods || !flags)
        ? contents
        : `(?${getGroupPrefix(atomic2, flags, state.useFlagMods)}${contents})`;
...
function getCasesOutsideCharClassRange(node, options) {
  ...
  const found = [];
  if (
    (min < 65 && (max === 65535 || max >= 131071)) ||
    (min === 65536 && max >= 131071)
  )
    return found;
  for (let i = min; i <= max; i++) {
    const char = cp(i);
    if (!charHasCase(char)) continue;
    const charsOutsideRange = getIgnoreCaseMatchChars(char).filter(
      (caseOfChar) => {
...
function getCodePointRangesFromChars(chars) {
  ...
  for (let i = 0; i < codePoints.length; i++)
    if (codePoints[i + 1] === codePoints[i] + 1) start ??= codePoints[i];
...
function createCaptureMap(hiddenCaptures, transfers) {
  ...
  for (const [to, from] of transfers)
    for (const num of from) getOrInsert(captureMap, num, {}).transferTo = to;
  return captureMap;
}
...
//#endregion
//#region ../../../node_modules/.bun/@shikijs+engine-javascript@4.4.1/node_modules/@shikijs/engine-javascript/dist/engine-compile.mjs
/**
...
//#endregion
//#region ../../../node_modules/.bun/@shikijs+engine-javascript@4.4.1/node_modules/@shikijs/engine-javascript/dist/engine-raw.mjs
/**
