import { t as toHtml } from './hast-util-to-html-D8qA7M9X.js';
//#region ../../../node_modules/.bun/@shikijs+types@4.4.1/node_modules/@shikijs/types/dist/index.mjs
...
var RegexSource = class {
  ...
  static replaceCaptures(regexSource, captureSource, captureIndices) {
    return regexSource.replace(
      ...
      (match, index, commandIndex, command) => {
...
var Theme = class {
  ...
  match(scopePath) {
    ...
    const effectiveRule = this._cachedMatchRoot
      .get(scopeName)
      .find((v) =>
...
function resolveParsedThemeRules(parsedThemeRules, _colorMap) {
  ...
  let root = new ThemeTrieElement(
    new ThemeTrieElementRule(0, null, -1, 0, 0),
    [],
  );
...
var EncodedTokenMetadata = class _EncodedTokenMetadata {
  ...
  static print(encodedTokenAttributes) {
    const languageId = _EncodedTokenMetadata.getLanguageId(
      encodedTokenAttributes,
    );
    const tokenType = _EncodedTokenMetadata.getTokenType(
      encodedTokenAttributes,
    );
    const fontStyle = _EncodedTokenMetadata.getFontStyle(
      encodedTokenAttributes,
    );
    const foreground = _EncodedTokenMetadata.getForeground(
      encodedTokenAttributes,
    );
    const background = _EncodedTokenMetadata.getBackground(
      encodedTokenAttributes,
    );
    console.log({
      languageId,
  ...
  static set(
    ...
    let _languageId = _EncodedTokenMetadata.getLanguageId(
      encodedTokenAttributes,
    );
    let _tokenType = _EncodedTokenMetadata.getTokenType(encodedTokenAttributes);
    let _containsBalancedBracketsBit =
      _EncodedTokenMetadata.containsBalancedBrackets(encodedTokenAttributes)
        ? 1
        : 0;
    let _fontStyle = _EncodedTokenMetadata.getFontStyle(encodedTokenAttributes);
    let _foreground = _EncodedTokenMetadata.getForeground(
      encodedTokenAttributes,
    );
    let _background = _EncodedTokenMetadata.getBackground(
      encodedTokenAttributes,
    );
...
function createMatchers(selector, matchesName) {
  ...
  function parseConjunction() {
    ...
    return (matcherInput) =>
      matchers.every((matcher2) => matcher2(matcherInput));
  }
  function parseInnerExpression() {
    ...
    return (matcherInput) =>
      matchers.some((matcher2) => matcher2(matcherInput));
  }
}
...
function parseInclude(include) {
  ...
  if (indexOfSharp === -1) return new TopLevelReference(include);
    ...
    return new TopLevelRepositoryReference(
...
var BeginEndRule = class extends Rule {
  ...
  compile(grammar, endRegexSource) {
    return this._getCachedCompiledPatterns(grammar, endRegexSource).compile(
      grammar,
    );
  }
  compileAG(grammar, endRegexSource, allowA, allowG) {
    return this._getCachedCompiledPatterns(grammar, endRegexSource).compileAG(
      grammar,
      allowA,
      allowG,
    );
...
var BeginWhileRule = class extends Rule {
  ...
  _getCachedCompiledWhilePatterns(grammar, endRegexSource) {
    ...
    if (this._while.hasBackReferences)
      this._cachedCompiledWhilePatterns.setSource(
        0,
    ...
    if (captures) {
      ...
      for (const captureId in captures) {
        ...
        if (captures[captureId].patterns)
          retokenizeCapturedWithRuleId = _RuleFactory.getCompiledRuleId(
            captures[captureId],
            helper,
            repository,
          );
          ...
          switch (reference.kind) {
            ...
            case 4:
              ...
              const externalGrammarInclude =
                reference.kind === 4 ? reference.ruleName : null;
              const externalGrammar = helper.getExternalGrammar(
                externalGrammarName,
                repository,
              );
              if (externalGrammar)
                if (externalGrammarInclude) {
                  let externalIncludedRule =
...
var RegExpSourceList = class {
  ...
  _resolveAnchors(onigLib, allowA, allowG) {
    return new CompiledRule(
      onigLib,
      this._items.map((e) => e.resolveAnchors(allowA, allowG)),
      this._items.map((e) => e.ruleId),
    );
  }
};
var CompiledRule = class {
  ...
  findNextMatchSync(string, startPosition, options) {
    const result = this.scanner.findNextMatchSync(
      string,
      startPosition,
      options,
    );
...
var BasicScopeAttributesProvider = class _BasicScopeAttributesProvider {
  ...
  _getBasicScopeAttributes = new CachedFn((scopeName) => {
    return new BasicScopeAttributes(
      this._scopeToLanguage(scopeName),
      this._toStandardTokenType(scopeName),
    );
  });
  /**
  ...
  _toStandardTokenType(scopeName) {
    const m = scopeName.match(
      _BasicScopeAttributesProvider.STANDARD_TOKEN_TYPE_REGEXP,
    );
    if (!m) return 8;
    switch (m[1]) {
      case 'comment':
...
function handleCaptures(
  ...
  for (let i = 0; i < len; i++) {
    ...
    if (captureRuleScopeName !== null) {
      const captureRuleScopesList = (
        localStack.length > 0
          ? localStack[localStack.length - 1].scopes
          : stack.contentNameScopesList
      ).pushAttributed(captureRuleScopeName, grammar);
      localStack.push(
...
var AttributedScopeStack = class _AttributedScopeStack {
  ...
  static fromExtension(namesScopeList, contentNameScopesList) {
    ...
    for (const frame of contentNameScopesList) {
      ...
      current = new _AttributedScopeStack(
        current,
        scopeNames,
  ...
  static createRoot(scopeName, tokenAttributes) {
    return new _AttributedScopeStack(
      null,
    ...
    if (this._emitBinaryTokens) {
      ...
      if (
        ...
        if (this.balancedBracketSelectors)
          containsBalancedBrackets =
            this.balancedBracketSelectors.match(scopes2);
      }
...
//#endregion
//#region ../../../node_modules/.bun/@shikijs+primitive@4.4.1/node_modules/@shikijs/primitive/dist/index.mjs
function resolveColorReplacements(theme, options) {
  const replacements =
    typeof theme === 'string' ? {} : { ...theme.colorReplacements };
...
async function normalizeGetter(p) {
  return Promise.resolve(typeof p === 'function' ? p() : p).then(
    (r) => r.default || r,
  );
}
/**
      ...
      (
        await Promise.all(
          langs
            ...
            .map(
              async (lang) =>
                await normalizeGetter(lang).then((r) =>
                  Array.isArray(r) ? r : [r],
                ),
            ),
        )
    ...
    while (alias[name]) {
      ...
      if (resolved.has(name))
        throw new ShikiError(
          `Circular alias \`${[...resolved].join(' -> ')} -> ${name}\``,
        );
      resolved.add(name);
    }
  }
  ...
  loadLanguage(lang) {
    ...
    const embeddedLazilyBy = new Set(
      [...this._langMap.values()].filter((i) =>
        i.embeddedLangsLazy?.includes(lang.name),
      ),
    );
    ...
    const g = this.loadGrammarWithConfiguration(
      lang.scopeName,
      1,
      grammarConfig,
    );
    ...
    if (missingLangs.length) {
      const dependents = langsGraphArray
        .filter(([_, lang]) => {
          ...
          return (lang.embeddedLanguages || lang.embeddedLangs)?.some((l) =>
            missingLangs.map(([name]) => name).includes(l),
          );
        })
...
function codeToTokensBase$1(primitive, code, options = {}) {
  ...
  if (
    isPlainLang(primitive.resolveLangAlias(options.lang || 'text')) ||
    isNoneTheme(themeName)
  )
...
function getLastGrammarState(...args) {
  ...
  const { lang = 'text', theme: themeName = primitive.getLoadedThemes()[0] } =
    options;
...
function _tokenizeWithTheme(code, grammar, theme, colorMap, options) {
  ...
  for (let i = 0, len = lines.length; i < len; i++) {
    ...
    if (includeExplanation && includeExplanation !== 'tokenType') {
      resultWithScopes = grammar.tokenizeLine(
        line,
        stateStack,
        tokenizeTimeLimit,
      );
...
function matchesOne(selector, scope) {
  return (
    selector === scope ||
    (scope.substring(0, selector.length) === selector &&
...
function codeToTokensWithThemes(
  ...
  code,
  options,
  codeToTokensBaseFn = codeToTokensBase$1,
) {
...
//#endregion
//#region ../../../node_modules/.bun/@shikijs+core@4.4.1/node_modules/@shikijs/core/dist/index.mjs
...
function flatTokenVariants(
  ...
  const styles = variantsOrder.map((t) =>
    getTokenStyleObject(merged.variants[t]),
  );
  const styleKeys = new Set(styles.flatMap((t) => Object.keys(t)));
  const mergedStyles = {};
  const varKey = (idx, key) => {
    const keyName =
      key === 'color' ? '' : key === 'background-color' ? '-bg' : `-${key}`;
    return (
      cssVariablePrefix + variantsOrder[idx] + (key === 'color' ? '' : keyName)
    );
  };
...
function transformerDecorations() {
      ...
      function normalizePosition(p) {
        if (typeof p === 'number') {
          if (p < 0 || p > shiki.source.length)
            throw new ShikiError(
              `Invalid decoration offset: ${p}. Code length: ${shiki.source.length}`,
            );
          return {
  ...
  return {
    ...
    tokens(tokens) {
      ...
      return splitTokens(
        tokens,
        getContext(this).decorations.flatMap((d) => [
          d.start.offset,
          d.end.offset,
        ]),
      );
    ...
    code(codeEl) {
      ...
      const lines = [...codeEl.children].filter(
        (i) => i.type === 'element' && i.tagName === 'span',
      );
      ...
      function applyLineSection(line, start, end, decoration) {
        ...
        if (startIndex === -1)
          throw new ShikiError(
            `Failed to find start index for decoration ${JSON.stringify(decoration.start)}`,
          );
        if (endIndex === -1)
          throw new ShikiError(
            `Failed to find end index for decoration ${JSON.stringify(decoration.end)}`,
          );
        const children = lineEl.children.slice(startIndex, endIndex);
        if (
          !decoration.alwaysWrap &&
          children.length === lineEl.children.length
        )
      ...
      const lineApplies = [];
      const sorted = ctx.decorations.sort(
        (a, b) =>
...
function verifyIntersections(items) {
  for (let i = 0; i < items.length; i++) {
    ...
    if (foo.start.offset > foo.end.offset)
      throw new ShikiError(
        `Invalid decoration range: ${JSON.stringify(foo.start)} - ${JSON.stringify(foo.end)}`,
      );
    for (let j = i + 1; j < items.length; j++) {
      const bar = items[j];
      const isFooHasBarStart =
        foo.start.offset <= bar.start.offset &&
        bar.start.offset < foo.end.offset;
      const isFooHasBarEnd =
        foo.start.offset < bar.end.offset && bar.end.offset <= foo.end.offset;
      const isBarHasFooStart =
        bar.start.offset <= foo.start.offset &&
        foo.start.offset < bar.end.offset;
      const isBarHasFooEnd =
        bar.start.offset < foo.end.offset && foo.end.offset <= bar.end.offset;
      if (
        isFooHasBarStart ||
        isFooHasBarEnd ||
        isBarHasFooStart ||
        ...
        throw new ShikiError(
          `Decorations ${JSON.stringify(foo.start)} and ${JSON.stringify(bar.start)} intersect.`,
        );
      }
    }
  }
...
function getTransformers(options) {
  const transformers = sortTransformersByEnforcement(
...
function dimColor(color) {
  ...
  if (hexMatch) {
    ...
    if (hex.length === 8) {
      ...
      const b = hex[2];
      const a = hex[3];
      return `#${r}${r}${g}${g}${b}${b}${Math.round(
        Number.parseInt(`${a}${a}`, 16) / 2,
      )
        .toString(16)
        .padStart(2, '0')}`;
...
function codeToTokens(primitive, code, options) {
  ...
  if ('themes' in options) {
    ...
    const themes = Object.entries(options.themes)
      ...
      .sort((a, b) =>
        a.color === defaultColor ? -1 : b.color === defaultColor ? 1 : 0,
      );
    if (themes.length === 0)
    ...
    if (
      defaultColor &&
      'light-dark()' !== defaultColor &&
      !themes.some((t) => t.color === defaultColor)
    )
      throw new ShikiError(
    ...
    const themeColorReplacements = themes.map((t) =>
      resolveColorReplacements(t.theme, options),
    );
    fg = mapThemeColors(
      themes,
      themeRegs,
    ...
    .map((t, idx) => {
      const value =
        applyColorReplacements(
          themeRegs[idx][property],
          themeColorReplacements[idx],
        ) || 'inherit';
...
function codeToHast(
  ...
  for (const transformer of getTransformers(options))
    input =
      transformer.preprocess?.call(transformerContext, input, options) || input;
  let { tokens, fg, bg, themeName, rootStyle, grammarState } = codeToTokens(
    primitive,
    input,
    options,
  );
  ...
  return tokensToHast(
    tokens,
    {
      ...
      themeName,
      rootStyle:
        options.rootStyle === false ? false : (options.rootStyle ?? rootStyle),
    },
    contextSource,
    grammarState,
  );
...
function tokensToHast(
  ...
  tokens.forEach((line, idx) => {
    ...
    if (structure === 'classic') {
      for (const transformer of transformers)
        lineNode =
          transformer?.line?.call(context, lineNode, idx + 1) || lineNode;
  ...
  if (structure === 'classic') {
    ...
    for (const transformer of transformers)
      transformedCode =
        transformer?.code?.call(context, transformedCode) || transformedCode;
    root.children = [];
...
function mergeWhitespaceTokens(tokens) {
  return tokens.map((line) => {
    ...
    line.forEach((token, idx) => {
      ...
      if (
        couldMerge &&
        RE_WHITESPACE_ONLY.test(token.content) &&
        line[idx + 1]
      ) {
...
function mergeAdjacentStyledTokens(tokens) {
  return tokens.map((line) => {
    ...
    for (const token of line) {
      ...
      const prevStyle = stringifyTokenStyle(
        prevToken.htmlStyle || getTokenStyleObject(prevToken),
      );
      const currentStyle = stringifyTokenStyle(
        token.htmlStyle || getTokenStyleObject(token),
      );
...
async function createHighlighterCore(options) {
  ...
  return {
    ...
    codeToTokensBase: (code, options) =>
      codeToTokensBase(primitive, code, options),
    codeToTokensWithThemes: (code, options) =>
      codeToTokensWithThemes(primitive, code, options),
...
function createHighlighterCoreSync(options) {
  ...
  return {
    ...
    codeToTokensBase: (code, options) =>
      codeToTokensBase(internal, code, options),
    codeToTokensWithThemes: (code, options) =>
      codeToTokensWithThemes(internal, code, options),
...
var getSingletonHighlighterCore = /* @__PURE__ */ makeSingletonHighlighterCore(
  ...
  async function createHighlighter(options) {
    ...
    function resolveTheme(theme) {
      ...
      if (typeof theme === 'string') {
        ...
        if (!bundle)
          throw new ShikiError(
            `Theme \`${theme}\` is not included in this bundle. You may want to load it from external source.`,
          );
        return bundle;
      }
      return theme;
...
function createSingletonShorthands(createHighlighter, config) {
  ...
  async function get(code, options) {
    const shiki = await getSingletonHighlighter({
      langs: [options.lang],
      themes:
        'theme' in options ? [options.theme] : Object.values(options.themes),
    });
    const langs = await config?.guessEmbeddedLanguages?.(
      code,
      options.lang,
      shiki,
    );
