...
export async function routeGenerator(
  ...
  const imports = [
    ...sortedRouteNodes.map((node) => {
      ...
      // The specifier doubles as the wrapper's dev identity key: a route-tree
      // hot swap re-evaluates this module, and the same specifier returns the
      // SAME wrapper (component identity preserved → unchanged routes don't
      // remount). Inert in prod (the wrapper ignores it outside dev).
      return `const ${
        node.variableName as string
      }Import = ${DYNAMIC_FN}(() => import('${specifier}'), '${specifier}')`;
    }),
  ].join('\n');
  ...
  const depAcceptLines = hotSpecifiers.length
    ? [
    ...
    : [];
  // Structural self-accept: when a route file is added/removed/renamed this
  // module itself is rewritten; vite re-evaluates it and hands the new module
  // here, and the new tree is swapped into the live router (re-matching the
  // current URL in place — no full reload). A failed re-evaluation passes no
  // module: escalate honestly.
  const hotBlock = [
    'if (import.meta.hot) {',
    ...depAcceptLines,
  ...
  const routeImports = [
    ...
    `import { createRoute, ${DYNAMIC_FN}${
      depAcceptLines.length ? ', __ossido__internal__applyRouteHot' : ''
    }, __ossido__internal__applyRouteTree } from '@ossido-labs/ossido'`,
