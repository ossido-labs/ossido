...
const RUST_STACK = `    at .tuono/../src/routes/rust-error.rs:8:5
   0: std::panicking::begin_panic
             at /rustc/abc/library/std/src/panicking.rs:689:5
  10: tuono_app::routes::rust_error::{{closure}}
             at ./.tuono/../src/routes/rust-error.rs:8:5
  15: ___rust_try`
...
describe('parseStack', () => {
  ...
  it('parses the Rust backtrace format, pairing symbols with locations', () => {
    ...
    expect(frames[0]).toEqual({
      fn: undefined,
      file: '.tuono/../src/routes/rust-error.rs',
      line: 8,
      column: 5,
    })
    ...
    expect(frames[2]?.fn).toBe('tuono_app::routes::rust_error::{{closure}}')
    expect(frames[2]?.file).toBe('./.tuono/../src/routes/rust-error.rs')
  ...
  it('treats rust stdlib/registry frames as non-application', () => {
    ...
    // `.tuono/../` is collapsed for readability.
