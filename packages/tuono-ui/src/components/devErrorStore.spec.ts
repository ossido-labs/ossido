...
beforeEach(() => {
  ...
  delete w['__TUONO_DEV_ERRORS__']
  delete w['__TUONO_DEV_ERRORS_BUFFER__']
...
describe('devErrorStore', () => {
  ...
  it('defaults the corner to bottom-left and persists a change', async () => {
    ...
    expect(globalThis.localStorage?.getItem('tuono:dev-overlay-corner')).toBe(
      'top-right',
    )
  })

  it('loads the persisted corner on init', async () => {
    globalThis.localStorage?.setItem('tuono:dev-overlay-corner', 'top-left')
    const store = await freshStore()
  ...
  it('drains build errors buffered before the store loaded', async () => {
    const w = window as unknown as Record<string, unknown>
    w['__TUONO_DEV_ERRORS_BUFFER__'] = [buildError]
    ...
    expect(w['__TUONO_DEV_ERRORS__']).toBe(store)
  })
})
