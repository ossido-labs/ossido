...
describe('apiClient', () => {
  it('types the response of `.json()` from the method + path', () => {
    expectTypeOf(apiClient.get('/api/pokemons/[name]', { params: { name: 'pikachu' } }))
      .resolves.toEqualTypeOf<TypedResponse<Pokemon>>()

    expectTypeOf(apiClient.get('/api/health').then((r) => r.json())).resolves.toBeUnknown()
  })

  it('requires `params` for dynamic routes and forbids them for static ones', () => {
    // Static route: callable with no options at all.
    expectTypeOf(apiClient.get('/api/health')).resolves.toEqualTypeOf<TypedResponse<unknown>>()
